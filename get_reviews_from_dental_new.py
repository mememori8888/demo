...
RETRYABLE_ACCOUNT_ERROR_TEXT = "customer is not active"
...
def parse_int_env(name: str, default: int, minimum: Optional[int] = None) -> int:
    ...
    try:
        ...
        print(f"WARN: {name} の値が不正です: {raw!r}。{default} を使用します。", file=sys.stderr)
        value = default

    if minimum is not None and value < minimum:
        print(f"WARN: {name} が最小値 {minimum} 未満です: {value}。{minimum} を使用します。", file=sys.stderr)
        value = minimum
    return value


# Trigger APIの通常リトライ（max_retries）とは別に、アカウント状態エラー時の再実行サイクルを制御する
MAX_TRIGGER_RERUNS = parse_int_env('TRIGGER_RERUNS', 2, minimum=0)
TRIGGER_RERUN_WAIT_SECONDS = parse_int_env('TRIGGER_RERUN_WAIT_SECONDS', 60, minimum=5)


def safe_read_csv(filepath):
    """
    フールプルーフ設計：複数のエンコーディングを自動で試し、
    文字コードエラーによるクラッシュを防ぐ安全なCSV読み込み関数。
    """
    encodings = ['utf-8-sig', 'utf-8', 'cp932', 'shift_jis', 'euc-jp']
    for enc in encodings:
        try:
            with open(filepath, 'r', encoding=enc) as f:
                # 一度全体を読み込んでデコード可能かチェック
                f.read()
                
            # デコード成功ならDictReaderで読み直して返す
            with open(filepath, 'r', encoding=enc) as f:
            ...
            logging.warning(f"⚠️ {filepath} の読み込み中にエラー ({enc}): {e}")
            continue
...
def validate_environment():
    ...
    input_lines = sum(1 for _ in open(INPUT_CSV, encoding='utf-8', errors='ignore'))
...
class BrightDataWebScraperReviews:
    ...
    def trigger_snapshot(self, urls_with_params: List[Dict]) -> str:
        ...
        for rerun_count in range(MAX_TRIGGER_RERUNS + 1):
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    response_text = (e.response.text if e.response is not None else "")[:500]
                    http_error_msg = f"HTTP Error {status_code}: {str(e)}"
                    is_retryable_account_state_error = (
                        status_code == 400 and RETRYABLE_ACCOUNT_ERROR_TEXT.lower() in response_text.lower()
                    )
                    ...
                    if is_retryable_account_state_error and rerun_count < MAX_TRIGGER_RERUNS:
                        next_rerun_count = rerun_count + 1
                        logging.warning(
                            f"♻️ BrightDataアカウント状態エラーを検知したため、"
                            f"トリガー再実行を行います ({next_rerun_count}/{MAX_TRIGGER_RERUNS})"
                        )
                        logging.warning(f"   Wait {TRIGGER_RERUN_WAIT_SECONDS}s before re-run...")
                        time.sleep(TRIGGER_RERUN_WAIT_SECONDS)
            ...
            if last_error is not None:
                raise last_error
            raise RuntimeError("Failed to trigger snapshot: retry loop ended unexpectedly")
...
def load_dental_csv():
    ...
    # ▼▼▼ 修正箇所：安全な読み込み関数を使用 ▼▼▼
    try:
        all_rows, _ = safe_read_csv(INPUT_CSV)
...
def load_existing_reviews():
    ...
    for row in all_rows:  # readerの代わりにall_rowsをループ
        if not has_facility_gid:
            row['施設GID'] = ''
        reviews.append(row)
