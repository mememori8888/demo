...
RETRYABLE_ACCOUNT_ERROR_TEXT_LOWER = RETRYABLE_ACCOUNT_ERROR_TEXT.lower()
...
TRIGGER_RERUNS = parse_int_env('TRIGGER_RERUNS', 2, minimum=0)
...
class BrightDataWebScraperReviews:
    ...
    def trigger_snapshot(self, urls_with_params: List[Dict]) -> str:
        ...
        for rerun_count in range(TRIGGER_RERUNS + 1):
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    is_retryable_account_state_error = (
                        status_code == 400 and RETRYABLE_ACCOUNT_ERROR_TEXT_LOWER in response_text.lower()
                    )
                    ...
                    if is_retryable_account_state_error and rerun_count < TRIGGER_RERUNS:
                        logging.warning(
                            f"♻️ BrightDataアカウント状態エラーを検知したため、"
                            f"トリガー再実行を行います ({rerun_count + 1}/{TRIGGER_RERUNS})"
                        )
