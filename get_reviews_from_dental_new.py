...
class BrightDataWebScraperReviews:
    ...
    def trigger_snapshot(self, urls_with_params: List[Dict]) -> str:
        ...
        while True:
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    is_retryable_account_state_error = (
                        status_code == 400 and "customer is not active" in response_text.lower()
                    )
            ...
            if should_rerun_cycle:
                continue
            break

        raise RuntimeError("Failed to trigger snapshot: retry loop exited unexpectedly")
