...
TRIGGER_RERUNS = max(0, int(os.getenv('TRIGGER_RERUNS', '2')))
TRIGGER_RERUN_WAIT_SECONDS = max(5, int(os.getenv('TRIGGER_RERUN_WAIT_SECONDS', '60')))
...
class BrightDataWebScraperReviews:
    ...
    def trigger_snapshot(self, urls_with_params: List[Dict]) -> str:
        ...
        for rerun_index in range(TRIGGER_RERUNS + 1):
            for attempt in range(max_retries):
                try:
                    ...
                    is_retryable_account_state_error = (
                        status_code == 400 and RETRYABLE_ACCOUNT_ERROR_TEXT in response_text.lower()
                    )
                    ...
                    if is_retryable_account_state_error and rerun_index < TRIGGER_RERUNS:
                        rerun_count = rerun_index + 1
                        logging.warning(
                            f"♻️ BrightDataアカウント状態エラーを検知したため、"
                            f"トリガー再実行を行います ({rerun_count}/{TRIGGER_RERUNS})"
                        )
