"""
BrightData API施設データ収集ツール（バッチ処理対応版）

wordpress_only_heatmap_pref_firstnum_optimized.csvの推奨リクエスト回数を使用し、
「県から最初の数字まで」列をそのまま検索アドレスとして使用。
GitHub Actionsで分割実行できるよう2000件単位で処理する機能を追加。
"""
import os
import requests
import json
...
def update_mini_batch(base_query, api_token, zone_name, facility_file, update_facility_path, exclude_gids_path,
    ...
    if not os.path.exists(log_dir):
        os.makedirs(log_dir)
    log_file_path = os.path.join(log_dir, "app_batch.log")
    ...
    # heat_map_path が指定されている場合はそれを使用、そうでなければ自動選択
    if heat_map_path and os.path.exists(heat_map_path):
        heatmap_file_path = heat_map_path
    else:
        # 自動選択：優先順位でファイルを探す
        # results/dental_heatmap_address.csv を最優先に追加
        heatmap_file_path = os.path.join('results', 'dental_heatmap_address.csv')
        if not os.path.exists(heatmap_file_path):
            heatmap_file_path = os.path.join('results', 'wordpress_only_heatmap_pref_firstnum_optimized.csv')
            if not os.path.exists(heatmap_file_path):
                heatmap_file_path = os.path.join('results', 'dental_heatmap_pref_firstnum.csv')
                if not os.path.exists(heatmap_file_path):
                    heatmap_file_path = os.path.join('results', 'wordpress_only_heatmap_pref_firstnum.csv')

    if os.path.exists(heatmap_file_path) and os.path.getsize(heatmap_file_path) > 0:
        print(f"📊 ヒートマップ集計を読み込み中: {heatmap_file_path}")
        try:
            with open(heatmap_file_path, newline="", encoding='utf-8') as hf:
        ...
        # heatmap ファイルが見つからない or 空の場合
        print(f"⚠️ ヒートマップファイルが見つからないか空です: {heatmap_file_path}")
        return 0

    # 住所リストの読み込み（ヒートマップのキーから）
    ...
    if start_index is not None and batch_size is not None:
        ...
        address_list = all_addresses[start_index:end_index]
        print(f"分割実行: {start_index}-{end_index-1} ({len(address_list)} 件) / 全 {len(all_addresses)} 件")
    else:
        address_list = all_addresses
        print(f"全件実行: {len(address_list)} 件")
    
    if not address_list:
        print("処理対象の住所データが見つかりません。")
        return 0
    ...
    if os.path.exists(facility_file) and os.path.getsize(facility_file) > 0:
        try:
            with open(facility_file, newline="", encoding='utf-8') as f:
                ...
                for r in reader:
                    ...
                    gid = r.get('施設GID') or ""
                    try:
                        fid = int(r.get('施設ID')) if r.get('施設ID') else None
                        if fid is not None:
                           ids.append(fid)
                    except Exception:
                        fid = None
    ...
    def process_address(addr, zone_name_for_thread):
        ...
        json_responses, query_request_count = search_places(
            ...
            query=search_query, # "東京都中央区銀座2 歯医者"
            log_dir=log_dir,
        ...
        for p in places:
            ...
            if not full_addr:
                 # 住所がない場合、元のクエリ（例：東京都中央区銀座2）を暫定的に入れる
                 full_addr = addr
    ...
    # 並列実行設定
    progress_path = os.path.join(results_dir, 'progress_batch.txt')
    cpu = os.cpu_count() or 2
    IO_MULTIPLIER = int(os.environ.get("IO_MULTIPLIER", "3"))
    MAX_WORKERS = int(os.environ.get("MAX_WORKERS", "1")) # デフォルトは1（シーケンシャル処理）
    ...
    print(f"並列処理設定: MAX_WORKERS={MAX_WORKERS}, 実際のworkers={workers}")
    
    all_rows = []
    if workers == 1:
        print(f"シーケンシャル処理を開始")
        ...
        # 並列処理
        print(f"並列処理を開始 (workers={workers})")
        with ThreadPoolExecutor(max_workers=workers) as executor:
            futures = {executor.submit(process_address, addr, zone_name): addr for addr in address_list}
    ...
    # リクエストログを出力（無効化）
    # request_log_path = os.path.join(results_dir, f"request_counts_batch_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.csv")
        ...
        try:
            with open('', 'w', newline='', encoding='utf-8') as f:
                ...
                for rec in request_log:
                    writer.writerow(rec)
            print(f"INFO: リクエストログを出力しました: {request_log_path}")
        except Exception as e:
    ...
    if fid_file_path and all_fid_rows:
        try:
            ...
            logging.info(f"fid_file に {len(all_fid_rows)} 件のFIDマッピングを書き込みました。")
            print(f"INFO: fid_file に {len(all_fid_rows)} 件のFIDマッピングを書き込みました。")
        except Exception as e:
    ...
    print(f"Finished batch processing. Appended {appended_count} new rows. Total found {len(all_rows)}. Requests: {total_requests_agg}")
    return total_requests_agg


def run_from_config_batch(config_file, start_index=None, batch_size=None, heat_map_path=None):
    try:
    ...
    print("BrightData Web Unlocker バッチ処理版で実行します。")
    ...
    for task in tasks:
        ...
        # コマンドライン引数 > 環境変数 > settings.json の優先順位
        if FACILITY_FILE_OVERRIDE:
            facility_csv = FACILITY_FILE_OVERRIDE
        else:
        ...
        # fid_file のパスを動的に生成（facility_file ベース）
        ...
        # heat_map のパス（優先順位: 引数 > 環境変数 > 設定ファイル）
        configured_heat_map_path = None
        if heat_map_path:
            configured_heat_map_path = heat_map_path
            print(f"DEBUG: Using parameter heat_map: {heat_map_path}")
        else:
            ...
            if heat_map_cfg:
                ...
                print(f"DEBUG: Using settings.json heat_map: {heat_map_cfg}")
            else:
                print(f"DEBUG: No heat_map specified")
        ...
        if configured_heat_map_path:
            print(f'設定されたヒートマップファイルは{configured_heat_map_path}')

        time.sleep(1)
        print(f"\n--- タスク '{task_name}' の処理を開始 ---")
        
        total_requests = update_mini_batch(
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='BrightData API施設データ収集ツール（バッチ処理版）')
    ...
    parser.add_argument('--heat-map', type=str, help='ヒートマップファイルのパス')
    parser.add_argument('--facility-file', type=str, help='出力施設ファイルのパス')
    
    args = parser.parse_args()
    
    # 環境変数とコマンドライン引数の両方をサポート（環境変数優先）
    CONFIG_FILE = os.environ.get('CONFIG_FILE', args.config)
    print(f"🚀 Starting with CONFIG_FILE: {CONFIG_FILE}")
    
    # START_INDEX: 環境変数 > コマンドライン引数
    if 'START_INDEX' in os.environ:
        START_INDEX = int(os.environ.get('START_INDEX'))
    elif args.start is not None:
        START_INDEX = args.start
    else:
        START_INDEX = None
    
    # BATCH_SIZE: 環境変数 > コマンドライン引数
    BATCH_SIZE = int(os.environ.get('BATCH_SIZE', args.batch_size))
    
    # HEAT_MAP: 環境変数 > コマンドライン引数（Issue Opsで確実に設定）
    HEAT_MAP = os.environ.get('HEAT_MAP') or args.heat_map
    
    # FACILITY_FILE: 環境変数 > コマンドライン引数（Issue Opsで確実に設定）
    FACILITY_FILE_OVERRIDE = os.environ.get('FACILITY_FILE') or args.facility_file
    
    print(f"DEBUG: 環境変数 HEAT_MAP={os.environ.get('HEAT_MAP')}")
    print(f"DEBUG: 引数 args.heat_map={args.heat_map}")
    print(f"DEBUG: 最終 HEAT_MAP={HEAT_MAP}")
    print(f"DEBUG: 環境変数 FACILITY_FILE={os.environ.get('FACILITY_FILE')}")
    print(f"DEBUG: 引数 args.facility_file={args.facility_file}")
    print(f"DEBUG: 最終 FACILITY_FILE_OVERRIDE={FACILITY_FILE_OVERRIDE}")
    
    # 総件数表示モード
    if args.total_count:
        # ヒートマップファイルの特定
        if HEAT_MAP and os.path.exists(HEAT_MAP):
            heatmap_path = HEAT_MAP
        else:
            # デフォルトの優先順位
            heatmap_path = os.path.join('results', 'dental_heatmap_address.csv')
        ...
        if os.path.exists(heatmap_path):
            ...
            print(f"エラー: ヒートマップファイルが見つかりません: {heatmap_path}")
        exit(0)
    ...
    if START_INDEX is not None:
        run_from_config_batch(CONFIG_FILE, START_INDEX, BATCH_SIZE, HEAT_MAP)
    else:
        run_from_config_batch(CONFIG_FILE, heat_map_path=HEAT_MAP)